...
class SimpleRegistration extends Component {
  ...
  componentDidMount() {
    this.assignMasks();
    this.applyCustomerName();
    this.applyShopIDFromLocalStorage();
  }

  applyShopIDFromLocalStorage() {
    const shop = JSON.parse(localStorage.getItem('shop'));
    if (shop) {
      this.setState({ storeID: shop.id });
    }
  }

  applyCustomerName() {
    const { customerName } = this.props.match.params;
    if (!customerName) return;
    this.setState({ socialName: Helper.applyUpperCase(customerName) });
  }
